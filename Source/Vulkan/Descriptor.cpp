...
void DescriptorSet::Update(u32 binding, u32 arrayElement, VkDescriptorType type, std::span<BufferUpdate> buffers)
{
	...
	{
		bInfos.push_back(
			VkDescriptorBufferInfo{ .buffer = buf.Buffer.GetHandle(), .offset = buf.Offset, .range = buf.Range });
	}
